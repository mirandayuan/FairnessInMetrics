...
for train in index_train:
    data['X_train'].append(X.iloc[train].tolist())
    data['y_train'].append(int(y.iloc[train]))
for test in index_test:
    data['X_test'].append(X.iloc[test].tolist())
    data['y_test'].append(int(y.iloc[test]))



### save -> json file
save(json.dumps(data), Data, 'w')
