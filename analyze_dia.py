...
def roc_plot(tpr, fpr, label, roc):
    ...
    for i in range(len(tpr)):
        plt.plot(tpr[i], fpr[i], color[i], label = label[i] % roc[i])
        i += 1
...
prepro = curve['prepro']
pre = curve['pre']
age_test = curve['age_test']
age_prepro = curve['age_prepro']
age_pre = curve['age_pre']
...
tpr, fpr, thre = roc_curve(y_test, prepro)
tpr_e = [tpr]
fpr_e = [fpr]
...
for i in range(len(age_test)):
    ...
    if 0 in age_pre[i] and 1 in age_pre[i]:
        print("auc score: ")
        age_auc.append(roc_auc_score(age_test[i], age_pre[i]))
        print(age_auc[i])
    else:
        print("only one class present")
    print("\n")

    tpr, fpr, thre = roc_curve(age_test[i], age_prepro[i])
    age_tpr.append(tpr)
    age_fpr.append(fpr)
